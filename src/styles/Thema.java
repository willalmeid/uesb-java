package styles;

import java.awt.Color;
import java.awt.Font;

public class Thema {

	public static final Color corPrincipal = new Color(52, 86, 120);
//	public static final Color corAuxiliar = new Color();
	public static final Color corAuxiliar2 = new Color(51, 51, 51);
	public static final Color corFundoPrincipal = new Color(239, 239, 239);
	public static final Color corBranco = new Color(255, 255, 255);
	
	public static final Color cortransparent = new Color(0, 0, 0, 0);
	
	public static final Font FONT_TITLE = new Font("Montserrat", Font.BOLD, 36);
	public static final Font FONT_LABELS_INPUTS = new Font("Montserrat", Font.BOLD, 14);
	public static final Font FONT_INPUTS = new Font("Montserrat", Font.PLAIN, 14);
}
